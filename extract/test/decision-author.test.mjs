...
import { files, source } from "../lib.mjs";
...
test("compile turns anchors into provenance and fills labels from the knob index", () => {
  ...
  // in whichever chunk this build puts it (chunk names change every release)
  const find = "reason:\"force_5m_env\"";
  const file = [...files.keys()].find(f => f.endsWith(".js") && source(f).includes(find));
  assert.ok(file, `no chunk in work/extracted contains ${find}`);
  ...
  const draft = { id: "t", title: "T", group: "G", question: "Q", shape: "first-wins", observe: "none",
    anchors: [{ file, find, span: "function" }],
    rungs: [{ id: "r", mechanism: "env", knob: "env-force-prompt-caching-5m", input: "toggle", effect: { value: "5m" }, anchors: [{ file, find }] }] };
  ...
  assert.equal(r.provenance[0].file, file);
