...
// The page for a list of decision records. Pure: the site tests compare the committed page
// with this function's output, so a page left behind by a decisions.json edit fails them.
export function renderWhatWins(items, knobs = knobIndex(root)) {
  const knobName = x => `\`${x.label ?? knobs.get(x.knob)?.title ?? x.knob}\``;
  const decisions = [...items].sort((a, b) => Number(surprising(b)) - Number(surprising(a)) || a.title.localeCompare(b.title));
...
if (process.argv[1] === new URL(import.meta.url).pathname) {
  ...
  writeFileSync(`${root}outputs/what-wins.md`, renderWhatWins(decisions));
